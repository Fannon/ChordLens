...
- This project is a Rust `nih-plug` MIDI plugin. `nih_export_vst3!` and `nih_export_clap!` expose the plugin entry points, while `cargo build --release` produces the shared library that gets packaged for hosts.
- VST3 and CLAP packaging details differ by host and platform. Treat raw renamed binaries as local convenience artifacts; verify final host compatibility before release.
