...
pub fn detect_scale(
    ...
    for (i, &m) in history.iter().enumerate() {
        counts[(m % 12) as usize] += if i > l / 2 { 2 } else { 1 };
    }
    let modes = [
    ...
    let mut bs = -1000;
    let mut bn = String::from("Unknown");
    for r in 0..12 {
        for (m, ints, w) in modes.iter() {
            ...
            for pc in 0..12 {
                let f = counts[pc as usize];
                if f == 0 {
                    continue;
                }
                let rel = (pc + 12 - r) % 12;
                if ints.contains(&rel) {
                    s += f * 5;
                } else {
                    s -= f * 50;
                }
    ...
    for r in 0..12 {
        for (m, ints, _) in modes.iter() {
            if format!("{} {}", pc_name(r as u8, r as u8), m) == bn {
                rp = r;
                bi = ints.clone();
            }
        }
